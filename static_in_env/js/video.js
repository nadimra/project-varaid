...
sizes.forEach((event) => {
  event.addEventListener("click", () => {
    ...
    console.log("Current Player: " + $("#video_player").width());
    console.log("Current container: " + $(".container").width());

    cropBorder.style.height = size + '%';
    cropBorder.style.width = size + '%';
    cropBorder.style.top = '0';
    cropBorder.style.left = '0';
    console.log("New Size: " + $("#crop-video").width());
...
editBtn.addEventListener('click',() => {
  ...
  if(checkBounds() == false){
    ...
    $.ajax(
      {
          type:"GET",
          url: "extractHighlight",
          data:{
            currentTime: mainVideo.currentTime,
            cropTop: cropBounds[0]+4,
